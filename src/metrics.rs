...
//! - `plumtree_peer_count` - Current total number of peers
...
pub fn init_metrics() {
    ...
    describe_gauge!("plumtree_peer_count", "Current total number of peers");
    ...
    // Priority queue metrics (gauges for current state)
    describe_gauge!(
    ...
    // Priority queue metrics (counters for throughput)
    describe_counter!(
        "plumtree_priority_enqueued_total",
        "Total messages enqueued by priority level"
    );
    describe_counter!(
        "plumtree_priority_dequeued_total",
        "Total messages dequeued by priority level"
    );
    describe_counter!(
        "plumtree_priority_dropped_total",
        "Total messages dropped (queue full) by priority level"
    );
    ...
    describe_counter!(
        "plumtree_chaos_events_total",
        "Total chaos events by kind (deliver, delay, drop, partition)"
    );

    // QUIC transport metrics
    describe_gauge!(
        "plumtree_quic_connections",
        "Current number of active QUIC connections"
    );
    describe_counter!(
        "plumtree_quic_bytes_sent_total",
        "Total bytes sent over QUIC transport"
    );
    describe_counter!(
        "plumtree_quic_bytes_received_total",
        "Total bytes received over QUIC transport"
    );
    describe_counter!(
        "plumtree_quic_streams_opened_total",
        "Total QUIC streams opened"
    );
    describe_counter!(
        "plumtree_quic_datagrams_sent_total",
        "Total QUIC datagrams sent"
    );
    describe_counter!(
        "plumtree_quic_datagrams_received_total",
        "Total QUIC datagrams received"
    );
    describe_counter!(
        "plumtree_quic_disconnections_total",
        "Total QUIC disconnections"
    );
    describe_counter!(
        "plumtree_quic_migrations_total",
        "Total QUIC connection migrations"
    );
    describe_counter!(
        "plumtree_quic_zero_rtt_total",
        "Total QUIC 0-RTT connections"
    );
}
...
/// Increment eager peers gauge by 1.
pub fn inc_eager_peers() {
    gauge!("plumtree_eager_peers").increment(1.0);
}

/// Decrement eager peers gauge by 1.
pub fn dec_eager_peers() {
    gauge!("plumtree_eager_peers").decrement(1.0);
}
...
/// Increment lazy peers gauge by 1.
pub fn inc_lazy_peers() {
    gauge!("plumtree_lazy_peers").increment(1.0);
}

/// Decrement lazy peers gauge by 1.
pub fn dec_lazy_peers() {
    gauge!("plumtree_lazy_peers").decrement(1.0);
}
...
pub fn set_total_peers(count: usize) {
    gauge!("plumtree_peer_count").set(count as f64);
}

/// Increment total peers gauge by 1.
pub fn inc_total_peers() {
    gauge!("plumtree_peer_count").increment(1.0);
}

/// Decrement total peers gauge by 1.
pub fn dec_total_peers() {
    gauge!("plumtree_peer_count").decrement(1.0);
}
...
/// Record a message being enqueued with priority.
pub fn record_priority_enqueue(priority: crate::priority::MessagePriority) {
    match priority {
        crate::priority::MessagePriority::Critical => {
            counter!("plumtree_priority_enqueued_total", "priority" => "critical").increment(1)
        }
        crate::priority::MessagePriority::High => {
            counter!("plumtree_priority_enqueued_total", "priority" => "high").increment(1)
        }
        crate::priority::MessagePriority::Normal => {
            counter!("plumtree_priority_enqueued_total", "priority" => "normal").increment(1)
        }
        crate::priority::MessagePriority::Low => {
            counter!("plumtree_priority_enqueued_total", "priority" => "low").increment(1)
        }
    }
}

/// Record a message being dropped due to queue full.
pub fn record_priority_drop(priority: crate::priority::MessagePriority) {
    match priority {
        crate::priority::MessagePriority::Critical => {
            counter!("plumtree_priority_dropped_total", "priority" => "critical").increment(1)
        }
        crate::priority::MessagePriority::High => {
            counter!("plumtree_priority_dropped_total", "priority" => "high").increment(1)
        }
        crate::priority::MessagePriority::Normal => {
            counter!("plumtree_priority_dropped_total", "priority" => "normal").increment(1)
        }
        crate::priority::MessagePriority::Low => {
            counter!("plumtree_priority_dropped_total", "priority" => "low").increment(1)
        }
    }
}

/// Record a message being dequeued (taken out) from the priority queue.
pub fn record_priority_dequeue(priority: crate::priority::MessagePriority) {
    match priority {
        crate::priority::MessagePriority::Critical => {
            counter!("plumtree_priority_dequeued_total", "priority" => "critical").increment(1)
        }
        crate::priority::MessagePriority::High => {
            counter!("plumtree_priority_dequeued_total", "priority" => "high").increment(1)
        }
        crate::priority::MessagePriority::Normal => {
            counter!("plumtree_priority_dequeued_total", "priority" => "normal").increment(1)
        }
        crate::priority::MessagePriority::Low => {
            counter!("plumtree_priority_dequeued_total", "priority" => "low").increment(1)
        }
    }
}

// NOTE: set_peer_health removed - use inc/dec functions instead for multi-node support
...
/// Record a chaos event with type label.
///
/// # Arguments
///
/// * `kind` - The kind of chaos event: "deliver", "delay", "drop", or "partition"
///
/// # Example
///
/// ```ignore
/// record_chaos_event("drop");
/// record_chaos_event("delay");
/// record_chaos_event("partition");
/// ```
///
/// Prometheus output:
/// ```text
/// plumtree_chaos_events_total{kind="drop"} 42
/// plumtree_chaos_events_total{kind="delay"} 156
/// plumtree_chaos_events_total{kind="partition"} 12
/// ```
pub fn record_chaos_event(kind: &'static str) {
    counter!("plumtree_chaos_events_total", "kind" => kind).increment(1);
}
...
/// Set the current number of active QUIC connections.
pub fn set_quic_connections(count: usize) {
    gauge!("plumtree_quic_connections").set(count as f64);
}

/// Increment the QUIC connections gauge by 1.
pub fn inc_quic_connections() {
    gauge!("plumtree_quic_connections").increment(1.0);
}

/// Decrement the QUIC connections gauge by 1.
pub fn dec_quic_connections() {
    gauge!("plumtree_quic_connections").decrement(1.0);
}

/// Record bytes sent over QUIC transport.
pub fn record_quic_bytes_sent(bytes: usize) {
    counter!("plumtree_quic_bytes_sent_total").increment(bytes as u64);
}

/// Record bytes received over QUIC transport.
pub fn record_quic_bytes_received(bytes: usize) {
    counter!("plumtree_quic_bytes_received_total").increment(bytes as u64);
}

/// Record a QUIC stream being opened.
pub fn record_quic_stream_opened() {
    counter!("plumtree_quic_streams_opened_total").increment(1);
}

/// Record a QUIC datagram being sent.
pub fn record_quic_datagram_sent() {
    counter!("plumtree_quic_datagrams_sent_total").increment(1);
}

/// Record a QUIC datagram being received.
pub fn record_quic_datagram_received() {
    counter!("plumtree_quic_datagrams_received_total").increment(1);
}

/// Record a QUIC disconnection.
pub fn record_quic_disconnection() {
    counter!("plumtree_quic_disconnections_total").increment(1);
}

/// Record a QUIC connection migration.
pub fn record_quic_migration() {
    counter!("plumtree_quic_migrations_total").increment(1);
}

/// Record a QUIC 0-RTT connection.
pub fn record_quic_zero_rtt() {
    counter!("plumtree_quic_zero_rtt_total").increment(1);
}
...
impl NodeMetrics {
    ...
    pub fn set_total_peers(&self, count: usize) {
        gauge!("plumtree_peer_count", "node" => self.node_id.clone()).set(count as f64);
    }
    ...
    /// Record a message being enqueued with priority.
    pub fn record_priority_enqueue(&self, priority: crate::priority::MessagePriority) {
        let priority_label = priority.name();
        counter!("plumtree_priority_enqueued_total", "node" => self.node_id.clone(), "priority" => priority_label).increment(1);
    }

    /// Record a message being dequeued with priority.
    pub fn record_priority_dequeue(&self, priority: crate::priority::MessagePriority) {
        let priority_label = priority.name();
        counter!("plumtree_priority_dequeued_total", "node" => self.node_id.clone(), "priority" => priority_label).increment(1);
    }

    /// Record a message being dropped due to queue full.
    pub fn record_priority_drop(&self, priority: crate::priority::MessagePriority) {
        let priority_label = priority.name();
        counter!("plumtree_priority_dropped_total", "node" => self.node_id.clone(), "priority" => priority_label).increment(1);
    }
    ...
    ///
    /// * `kind` - The kind of chaos event: "deliver", "delay", "drop", or "partition"
    pub fn record_chaos_event(&self, kind: &'static str) {
        counter!("plumtree_chaos_events_total", "node" => self.node_id.clone(), "kind" => kind)
            .increment(1);
    }
}
