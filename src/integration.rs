...
impl<I: IdCodec> BroadcastEnvelope<I> {
    ...
    ///
    /// **Note**: Payload compression is now handled automatically by the Plumtree core.
    /// This method is kept for backward compatibility but the payload inside the message
    /// is already compressed if compression is enabled.
    ///
    ...
    #[deprecated(
        since = "0.2.0",
        note = "Compression is now handled automatically by Plumtree core"
    )]
...
impl<I: IdCodec> UnicastEnvelope<I> {
    ...
    #[allow(dead_code)] // Used by MemberlistStack::start() when tokio feature is enabled
    pub(crate) fn target(&self) -> &I {
        &self.target
    }
    ...
    SS: SyncStrategy<I, S>,
{
    ...
    pub async fn recv_outgoing_encoded(&self) -> Option<Bytes> {
        ...
        // Payload compression is handled by Plumtree core, just encode the envelope
    ...
    pub async fn run_storage_prune(&self) {
    ...
    pub async fn run_anti_entropy_sync<T>(&self, transport: T)
    where
        T: crate::Transport<I>,
    {
    ...
    S: MessageStore + 'static,
{
    fn on_deliver(&self, message_id: MessageId, payload: Bytes) {
        // The payload here is already decompressed (user data without compression header).
        // For storage and sync, we need to add the compression header so that when
        // messages are synced to other nodes, they can be properly processed by handle_gossip.
        //
        // Header byte 0 = not compressed (raw payload follows)
        use bytes::BufMut;
        let mut payload_with_header = bytes::BytesMut::with_capacity(1 + payload.len());
