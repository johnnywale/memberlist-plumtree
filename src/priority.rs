...
/// A snapshot of priority queue statistics.
///
/// This is a cloneable copy of the statistics at a point in time.
#[derive(Debug, Clone, Default)]
pub struct PriorityQueueStatsSnapshot {
    /// Messages enqueued per priority [Critical, High, Normal, Low]
    pub enqueued: [u64; 4],
    /// Messages dequeued per priority [Critical, High, Normal, Low]
    pub dequeued: [u64; 4],
    /// Messages dropped (queue full) per priority [Critical, High, Normal, Low]
    pub dropped: [u64; 4],
}

impl PriorityQueueStatsSnapshot {
    /// Get enqueued count for a priority
    pub fn enqueued(&self, priority: MessagePriority) -> u64 {
        self.enqueued[priority.index()]
    }

    /// Get dequeued count for a priority
    pub fn dequeued(&self, priority: MessagePriority) -> u64 {
        self.dequeued[priority.index()]
    }
