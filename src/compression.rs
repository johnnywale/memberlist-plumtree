...
/// Compress a payload for broadcast.
///
/// If compression is enabled and the payload is large enough, compresses the data
/// and prepends a header byte indicating the compression algorithm.
///
/// # Wire Format
/// - Byte 0: flags (bit 0 = compressed, bits 1-2 = algorithm)
/// - Bytes 1..N: payload data (compressed or raw)
///
/// # Returns
/// - `(compressed_payload, Some((original_size, compressed_size)))` if compression was applied
/// - `(header + original_payload, None)` if compression was skipped
pub fn compress_payload(
    ...
    if !config.should_compress(original_size) {
        // No compression - prepend flags byte (0 = not compressed)
        let mut result = bytes::BytesMut::with_capacity(1 + original_size);
        result.put_u8(0); // flags: not compressed
        result.put_slice(payload);
        return (result.freeze(), None);
    }

    // Try to compress
    match compress(payload, config.algorithm) {
        Ok(compressed) => {
            let compressed_size = compressed.len();

            // Only use compression if it actually saves space
            if compressed_size < original_size {
                let flags = flags::encode(true, config.algorithm.wire_id());
                let mut result = bytes::BytesMut::with_capacity(1 + compressed_size);
                result.put_u8(flags);
                result.put_slice(&compressed);
                (result.freeze(), Some((original_size, compressed_size)))
            } else {
...
/// Decompress a payload received from the network.
///
/// Checks the header byte and decompresses if needed.
///
/// # Arguments
...
pub fn decompress_payload(payload: &[u8]) -> Result<Bytes, CompressionError> {
    ...
    let flags_byte = payload[0];
    let data = &payload[1..];

    let (compressed, algorithm_id) = flags::decode(flags_byte);
