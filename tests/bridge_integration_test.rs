...
async fn test_full_receive_integration() {
    ...
    // Note: Network payloads include a compression header byte (0 = not compressed)
    let raw_content = b"Hello, this is a real message!";
    let mut payload_with_header = bytes::BytesMut::with_capacity(1 + raw_content.len());
    payload_with_header.put_u8(0); // compression flags: not compressed
    payload_with_header.put_slice(raw_content);
    ...
    // Verify the delivered message content (decompressed, without header byte)
    assert_eq!(delegate.delivered_count(), 1);
    let messages = delegate.get_messages();
    assert_eq!(messages[0].0, msg_id);
    assert_eq!(messages[0].1.as_ref(), raw_content);
